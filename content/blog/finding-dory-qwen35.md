...
Chunks of the images (e.g. 250 images) are sent to the model with prompt to predict the frame index for the question. Those frame indices are converted to xyz coordinates. 
...
FindingDory effectively evaluates in-context memories of VLAs for indoor household tasks in the habitat sim. It uses PDDL specifications to convert the frames index (given by the VLA) to the location of the target.
